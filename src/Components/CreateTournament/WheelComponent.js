 ...
 const WheelComponent = ({
  ...
  useEffect(()=>{
    ...
    if (!canvasInit) {
      ...
      setCanvasInit(true);
      wheelInit(newCanvasContext);
    }
    else {
      wheelInit(canvasContext);
    }

  }, [segments, canvasContext])

  const wheelInit = (newCanvasContext) => {
  ...
  const spin = (ctx) => {
    ...
    if (timerHandle === 0) {
      ...
      maxSpeed = Math.PI / latestSegs.current.length
      frames = 0
      
      timerHandle = setInterval(() => onTimerTick(ctx), timerDelay)
    }
  }

  const onTimerTick = (ctx) => {
    frames++    
    draw(ctx)
  ...
  const drawNeedle = (ctx) => {
    ...
    let i =
      latestSegs.current.length -
      Math.floor((change / (Math.PI * 2)) * latestSegs.current.length) -
      1
    if (i < 0) i = i + latestSegs.current.length
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillStyle = primaryColor || 'black'
    ctx.font = 'bold 1.5em proxima-nova'
    currentSegment = latestSegs.current[i]
